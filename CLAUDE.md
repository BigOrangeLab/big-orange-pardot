...
| `src/pardot-form/render.php`     | PHP template — wraps `$content` (rendered inner blocks) in `<form action="...">` + inline submit button + hidden attribution inputs; emits field and button CSS custom props via `get_block_wrapper_attributes()`. When `pardotFormUrl` is empty: silently returns nothing for visitors; renders a notice + a non-submittable preview form (no `action`, `onsubmit="return false;"`) for users with `edit_post` capability.                                                                                                                                                                                                                                                                                      |
...
| `src/attribution.js`             | Global cookie capture + hidden field population + client-side submit validation (HTML5 Constraint Validation API) + Pardot error-redirect handling (parses `errors=true` URL params, shows `.bol-pardot-errors` notice, re-populates visible fields) (built to `build/attribution.js`, enqueued on every frontend page)                                                                                                                                                                                                                                                                                                                                                                                           |
| `src/admin-bar-attribution.js`   | Admin bar "Clear all cookies" click handler — expires all 9 attribution cookies and reloads (built to `build/admin-bar-attribution.js`, enqueued via `bol_enqueue_admin_bar_assets()`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `src/log-viewer.js` | Logs tab entrypoint — mounts the DataViews-based log viewer app (`src/log-viewer-app.js`) and imports `src/log-viewer.scss`, built to `build/log-viewer.js` + `build/log-viewer.css` |
| `src/log-viewer-app.js` | Logs tab React/DataViews app — status badges, failed-only toggle, and per-entry payload inspector |
| `src/log-viewer.scss` | Logs tab styles — DataViews table import plus badge/inspector styling |
...
| `src/pardot-field/edit.js`    | InspectorControls (field settings + linked Field Styling panel that reads/writes the _parent_ block's style attributes via `getBlockRootClientId`+`updateBlockAttributes`)   |
...
| `includes/class-bol-admin-page.php` | Settings page (Settings submenu) — three tabs: **Settings** (credentials, OAuth connect/disconnect, logging toggles, form handler inspector), **Help** (user-facing setup documentation), and **Logs** (view/delete API log) |

### Pardot API integration

- **OAuth flow:** Salesforce authorization code flow. Credentials (`client_id`, `client_secret`, `business_unit_id`) stored in `wp_options` (autoload off). Tokens refreshed automatically by `BOL_Pardot_API::get_access_token()` when within 60 seconds of expiry. The plugin can optionally request Salesforce `api` scope to query `PardotBusinessUnit` records and auto-populate the Business Unit selector in settings (cached for 15 minutes).
- **API logging:** Optional request logging can be enabled in settings (`big_orange_pardot_enable_api_logging`). Each Salesforce/Pardot request writes a JSON line to `wp_upload_dir()['basedir']/big-orange-pardot-api.log` with sensitive values redacted. The Logs tab parses entries and renders a DataViews table with per-row inspection, and provides a one-click delete action that also disables logging.
...
- **REST endpoints:**
    ...
    - `GET /wp-json/big-orange-pardot/v1/form-handler-fields?handler_id={id}` — requires `manage_options`. Powers auto-insert on handler select, "Add missing field(s)", and "Replace all with Pardot fields" in the editor.
...
- **Attribution fields:** 10 hidden fields submitted with every form. 9 are cookie-backed and listed in `BOL_Pardot_API::ATTRIBUTION_FIELDS`: `utm_source`, `utm_medium`, `utm_campaign`, `utm_term`, `utm_content`, `referrer_url`, `landing_page_url`, `gclid`, `visitor_id` (Pardot tracking script cookie). The 10th, `last_form_submission_url`, is set directly from `window.location.href` on each page load (not a cookie) and is not in `ATTRIBUTION_FIELDS`.
- **Admin bar inspector:** `bol_register_admin_bar_node()` in `big-orange-pardot.php` adds an "Attribution (N)" menu to the WP admin bar (visible to `manage_options` users only, on both frontend and wp-admin). PHP reads `$_COOKIE` for the 8 attribution values; `src/admin-bar-attribution.js` handles the "Clear all cookies" button (expires cookies + reloads). Styles in `src/admin-bar-attribution.scss` (imported from JS, built to `build/admin-bar-attribution.css`); both enqueued by `bol_enqueue_admin_bar_assets()` (hooked to `wp_enqueue_scripts` and `admin_enqueue_scripts`).
