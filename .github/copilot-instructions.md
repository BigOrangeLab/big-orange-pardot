...
- `src/`: Source of truth for block code (JSX/SCSS/PHP render templates).
  - `src/pardot-form/`: Parent form block (`bigorangelab/pardot-form`) — handler dropdown, "Import fields from Pardot" button, `useInnerBlocksProps` with default 7-field template. Stores shared field style attributes (`fieldLabelColor`, `fieldInputBg`, `fieldBorderColor`, `fieldFocusColor`, `fieldBorderRadius`) and emits them as `--bol-*` CSS custom properties on the wrapper. Also owns all submit button attributes (`submitLabel`, `buttonTextColor`, `buttonBgColor`, `buttonBgGradient`, `buttonHoverBgColor`, border/padding/shadow/alignment attrs) — the submit button is rendered inline in the parent block, not as a separate child block. Native `color`/`spacing`/`border` supports for form-level appearance. `save.js` returns `<InnerBlocks.Content />` (not `null`) to serialize inner blocks.
